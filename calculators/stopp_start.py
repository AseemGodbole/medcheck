...
STOPP_DATABASE: list[STOPPEntry] = [
    ...
    STOPPEntry(
        ...
        condition="Non-valvular atrial fibrillation with CHA₂DS₂-VASc score of 0 (males) or 1 (females)",
...
START_DATABASE: list[STARTEntry] = [
    ...
    STARTEntry(
        ...
        indication="Atrial fibrillation with CHA₂DS₂-VASc score ≥ 2 (males) or ≥ 3 (females)",
...
def print_start_report(conditions: list):
    ...
    print("=" * 70)
    if not conditions:
        print("  ℹ️  No conditions provided — START criteria require known diagnoses.")
        print("     Pass conditions with --conditions flag when using combined_checker.py")
    elif not hits:
        print(f"  ℹ️  No START criteria matched for: {', '.join(conditions)}")
    else:
        for e in hits:
            print(f"\n  💊 [{e.criterion_id}] {e.section}")
